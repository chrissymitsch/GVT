var app = (function() {
    ...
    function initShaderProgram() {
		...
		if (shader == "toon") {
			vs = initShader(gl.VERTEX_SHADER, "vertexshader-toon");
			fs = initShader(gl.FRAGMENT_SHADER, "fragmentshader-toon");
		} else {
			vs = initShader(gl.VERTEX_SHADER, "vertexshader-phong");
			fs = initShader(gl.FRAGMENT_SHADER, "fragmentshader-phong");
		}
		/**
		 * END EA8
		 */
    ...
    function initUniforms() {
		...
		prog.numberOfColorsUniform = gl.getUniformLocation(prog, "numberOfColors");
    }
    ...
    function initEventHandler() {
        ...
        var deltaRotateLight = Math.PI / 36;
        var deltaTranslate = 0.05;
        var deltaScale = 0.05;
		
		// Default positions of lights.
		// Orbit 7 times for nice default light location.
		for (var i = 0; i < 7; i++) {
			orbitLights();
		}
		...
		/**
		 * Orbit calculation for lights.
		 */
		function orbitLights() {
			var signs = [1, -1];
			var radius = 3;
			for (var i = 0; i < illumination.light.length; i++) {
				illumination.light[i].position[0] = signs[i] * Math.cos(deltaRotateLight) * radius;
				illumination.light[i].position[2] = signs[i] * Math.sin(deltaRotateLight) * radius;
				
				deltaRotateLight += deltaRotate;
			}
		}
    }
    ...
    function render() {
		...
		gl.uniform1f(prog.numberOfColorsUniform, userInputNumberOfColors);
        // Loop over light sources.
        for (var j = 0; j < illumination.light.length; j++) {
